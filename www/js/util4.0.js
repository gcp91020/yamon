...
function byGN(a,b) {
	...
	var n1,n2
	if (a in devices) {
		n1 = devices[a].group.toLowerCase()
	} else {
		return (-1)
	}
	if (b in devices) {
		n2 = devices[b].group.toLowerCase()
	} else {
		return (1)
	}
...
function nudge(msg){
	...
	$('.dismiss button').unbind('click').click(function(){
		var request=$.ajax({
			...
			type: "GET",
			// url: "https://usage-monitoring.com/"+"current/dismiss"+_file_version+".php",
			// type: "POST",
...
function getMessage(msg, more){
	...
	var request=$.ajax({
			...
			// url: "https://usage-monitoring.com/"+"current/getMSG.php",
			type: "GET",
