...
function loadMonthly(){
	...
	function getMonthlyData(myr, mn, da){
		...
		.done(function(dlist,textStatus){
			...
			// xy
			if (!(dn in monthly_totals.usage)) {
				monthly_totals.usage[dn]={down:0,up:0,ul_down:0,ul_up:0}
			}
...
function loadHourly(cleardata){
	...
	.done(function(dlist,textStatus){
		...
		if(typeof(users_updated)==='undefined') var users_updated=''
                if(typeof(hourly_updated)==='undefined') var hourly_updated=''
...
function loadView(cleardata){
	...
	switch(cvs){
		...
		case 'daily-conn-tab':
			if(cleardata){
				$('#DailyConnData .is_d').addClass('clear')
			}
			setDailyConnTab()
			break;
...
function setDailyConnTab(){
	console.log("Conn tab clicked");
}
