...
CheckMAC2GroupinUserJS(){
	...
	ChangeMACGroup(){
		...
		local matchingMACs=$(cat "$_usersFile" | grep -e "^mac2ip" | grep "$1" | grep "\"active\":\"1\"")
		IFS=$'\n'
		for line in $matchingMACs ; do
			...
			for rule in $matchingRules ; do
				...
				#XY
				local src_add=$(echo $rule | awk '{print $5}')
				local dst_add=$(echo $rule | awk '{print $6}')
				eval $cmd -R ${YAMON_IPTABLES} $ln -s $src_add -d $dst_add -g $groupChain "$_iptablesWait"
...
CheckMAC2IPinUserJS(){
	...
	AddNewMACIP(){
		...
		#XY speical for 10.249.x.x
		local network_128="10.249."
		local WanGwIP=$(ip route | grep "dev wan proto static src" | grep "default via" | awk '{print $3}')
		if [[ "$WanGwIP" == "$network_128"* ]]; then
			#device=wan
			local wanGwArp=$(cat /proc/net/arp | grep "$WanGwIP " | awk '{print $4}')
			local wanIfIP=$(ip route | grep "dev wan proto static src" | grep "default via" | awk '{print $NF}' | head -n 1)
			local wanIfArp=$(ifconfig wan | grep "wan       Link encap:Ethernet  HWaddr" | awk '{print $NF}' | head -n 1)

			if [[ "$m" == "$wanIfArp" ]] ; then
			#only trigger when WAN IP changed, remove old wanIfIp, add new
				# weworkgw=$(ip route | grep "dev wan proto static src" | grep "default via" | awk '{print $3}')
				# weworkgwarp=$(cat /proc/net/arp | grep "$weworkgw " | awk '{print $4}')
				local wanGwLine=$(cat "$_usersFile" | grep $wanGwArp | grep $WanGwIP | tail -n 1)
				local wanIfLine=$(cat "$_usersFile" | grep $wanIfArp | grep $wanIfIP | tail -n 1)
				sed -i "/^mac2ip({ .*-${network_128}.*/d" $_usersFile
				[ -n "$wanGwLine" ] && sed -i "s~//MAC -> IP~//MAC -> IP\n$wanGwLine~g" "$_usersFile"
				[ -n "$wanIfLine" ] && sed -i "s~//MAC -> IP~//MAC -> IP\n$wanIfLine~g" "$_usersFile"

				# local network_128_Unknown_MACs=$(cat "$_usersFile" | grep '"group":"Unknown"' | awk -F\" '{print $4}')
				# for the_mac in $network_128_Unknown_MACs
				# do
				# 	found=$(cat "$_usersFile" | grep 'mac2ip({ "id"' | grep $the_mac)
				# 	if [ -z "$found" ]; then
				# 		sed -i "/^mac2group({ \"mac\":\"${the_mac}.*/d" $_usersFile
				# 		Send2Log "$the_mac deleted from users.js" 1
				# 	fi
				# done
		...
		# if [ "$m" == "20:76:93:60:02:15" ] ; then
		# 	sed -i "/^mac2ip({ \"id\":\"20:76:93:60:02:15-10.249.*/d" $_usersFile
		# 	weworkgw=$(ip route | grep "dev wan proto static src" | grep "default via" | awk '{print $3}')
		# 	weworkgwarp=$(cat /proc/net/arp | grep "$weworkgw " | awk '{print $4}')
		# 	weworkgwline=$(cat "$_usersFile" | grep $weworkgwarp | grep $weworkgw)
		# 	sed -i "/^mac2ip({ .*-10.249.*/d" $_usersFile
		# 	sed -i "s~//MAC -> IP~//MAC -> IP\n$weworkgwline~g" "$_usersFile"
		# fi
...
AddActiveDevices(){
	...
	do
		...
		#XY avoid duplicate IPs with same MAC
		if [ -z "$(cat "$macIPFile" | grep "$mac")" ] ; then
