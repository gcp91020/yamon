...
GetTraffic(){
	...
	do
		...
		if [ "$_generic_ipv4" == "$ip" ] || [ "$_generic_ipv6" == "$ip" ] ; then
			...
			#XY
			local do1=$(echo "$ipt" | cut -d' ' -f1)
			Send2Log "GetTraffic: do1 $do1 " 1

			local do=$(echo -e "$ipt" | head -n 1 |cut -d' ' -f1)
			...
			[ -n "$wl" ] && IFS=$'\n' && for the_number in "$wl"; do iptables -D "$YAMON_IPTABLES" $the_number; done
			wr=$(iptables -L "$YAMON_IPTABLES" -n --line-numbers | grep RETURN | awk '{ print $1 }')
			#[ -n "$wr" ] && iptables -D "$YAMON_IPTABLES" "$wr"
			[ -n "$wr" ] && IFS=$'\n' && for the_number in "$wr"; do iptables -D "$YAMON_IPTABLES" $the_number; done
			...
			if [ -z "$mac" ] ; then
				...
				CheckMAC2IPinUserJS "$mac" "$ip"
				local groupName=$(GetDeviceGroup "$mac" "$ip")
				CheckMAC2GroupinUserJS "$mac" "$groupName"
				CheckIPTableEntry "$ip" "$groupName"
	...
	intervalTraffic=$(echo -e "$intervalTraffic" | grep -e "^hourlyData4({ .* })$" | grep -v "un:kn:ow:n0:0m:ac" | grep -v "192.168.238")

	local hrlyData=$(cat "$hourlyDataFile" | grep -v "un:kn:ow:n0:0m:ac" | grep -v "192.168.238")

	local currentUptime=$(cat /proc/uptime | cut -d' ' -f1)
	[ -z "$currentUptime" ] && Send2Log "GetTraffic: currentUptime is null?!?" 2

	#Send2Log "XY currentUptime _uptime $currentUptime $_uptime" 1

	if [ -n "$currentUptime" ] ; then
		currentUptime1=${currentUptime%.*}
		_uptime1=${_uptime%.*}
		if [[ $currentUptime1 -lt $_uptime1 ]] ; then
	...
	#Send2Log "XY after currentUptime _uptime $currentUptime $_uptime" 1
	#Send2Log "IndentList $IndentList" 1

	local interfaceTotals=$(GetInterfaceTraffic)
	#Send2Log "XY interfaceTotals $interfaceTotals" 1
	local memoryTotals=$(GetMemory)
	#Send2Log "XY memoryTotals $memoryTotals" 1
	local disk_utilization=$(df "${d_baseDir}" | tail -n 1 | awk '{print $5}')
	#Send2Log "XY disk_utilization $disk_utilization" 1
	...
	#Send2Log "XY intervalTraffic $intervalTraffic totalsLine $totalsLine " 1

	if [ -n  "$intervalTraffic" ] ; then
		# Send2Log "XY IndentList $(IndentList "$intervalTraffic\n$totalsLine") " 1
		# Send2Log "XY hrlyData $hrlyData " 1
		# Send2Log "XY totalsLine ${totalsLine//,0,0\"/\"}\n" 1
		# Send2Log "XY intervalTraffic ${intervalTraffic//,0,0\"/\"}\n" 1
		# Send2Log "XY hrlyData2 $(echo "$hrlyData" | grep -v "\"hour\":\"$hr\"")\n" 1
		# Send2Log "hourlyDataFile $hourlyDataFile" 1

		# echo -e "$(echo "$hrlyData" | grep -v "\"hour\":\"$hr\"")\n${intervalTraffic//,0,0\"/\"}\n${totalsLine//,0,0\"/\"}" > "$hourlyDataFile"
		# when the amount of the lines of intervalTraffic, echo -e "${intervalTraffic//,0,0\"/\"}\n" > $hourlyDataFile will fail
		intervalTraffic="${intervalTraffic//,0,0\"/\"}"
		totalsLine="${totalsLine//,0,0\"/\"}"
		echo -e "$(echo "$hrlyData" | grep -v "\"hour\":\"$hr\"")\n${intervalTraffic}\n${totalsLine}" > "$hourlyDataFile"

		# Send2Log "XY ${mm#0} - ${_updateTraffic:-4} rawtraffic_hr $rawtraffic_hr" 1
		# Send2Log "XY (${mm#0} - ${_updateTraffic:-4}) (hourlyDataFile --> $hourlyDataFile)" 1
		...
		#IndentList_n=$(IndentList "$intervalTraffic\n$totalsLine")
		#Send2Log "XY IndentList $(IndentList "$intervalTraffic\n$totalsLine") " 1
		# Send2Log "XY ($hr:$mm --> $vnx) " 1
		# Send2Log "XY ($hr:$mm --> $vnx) (ltrl --> $ltrl) (rawtraffic_hr $rawtraffic_hr)" 1
		# Send2Log "XY GetTraffic ($hr:$mm --> $vnx) (IndentList_n --> $IndentList_n) " $ltrl
		# Send2Log "XY 1(${mm#0} - ${_updateTraffic:-4}) $hr:$mm ($vnx)\n $rawtraffic_hr" 1
		echo -e "\n//$hr:$(printf %02d $(( ${mm#0} - ${_updateTraffic:-4} )))->$hr:$mm ($vnx)\n" >> "$rawtraffic_hr"
		# Send2Log "XY 2 ${intervalTraffic//,0,0\"/\"}\n" 1
		# Send2Log "XY 3 ${totalsLine//,0,0\"/\"}" 1
		# echo -e "${intervalTraffic//,0,0\"/\"}\n${totalsLine//,0,0\"/\"}" >> "$rawtraffic_hr"
		echo -e "${intervalTraffic}\n${totalsLine}" >> "$rawtraffic_hr"
		# Send2Log "XY 2 (${intervalTraffic}  $totalsLine)" 1
	else
	...
	# Send2Log "XY totlasline $totalsLine" 1
	...
	# Send2Log "XY hourlyDataFile $hourlyDataFile" 1

	cp "$hourlyDataFile" "$_path2CurrentMonth"
	cp "$tmpLastSeen" "${_lastSeenFile}"

	# Send2Log "XY hourlyDataFile $hourlyDataFile _path2CurrentMonth $_path2CurrentMonth tmpLastSeen $tmpLastSeen _lastSeenFile ${_lastSeenFile}" 1
